...
describe('userController unit tests', () => {
  ...
  before(() => {
    ...
    server = require(path.join(__dirname, '..', 'server.js', { bustCache: true }))
  })
  ...
  it('Delete datas, should return 200 and value 1', (done) => {
    ...
    .end((err, res) => {
      if (err) {
        return done(new Error(`${err}-${res.error.text}`))
      } else if (parseInt(res.body, 10) !== 1) {
        return done(new Error(`DELETE /user must return 1, ${parseInt(res.body, 10)} returned`));
      }
